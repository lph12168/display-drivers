		...
		struct drm_display_mode *mode)
{
	...
	ret = sde_connector_get_mode_info(&sde_conn->base, mode, &mode_info);
	if (ret)
		return ret;
		...
		void *display, const struct msm_resource_caps_info *avail_res)
{
	...
	if (!(drm_mode->private_flags & MSM_MODE_FLAG_SHARED_DISPLAY)) {
		...
		sde_connector_get_mode_info(shd_display->base->connector,
				&shd_display->base->mode, &base_mode_info);
		...
		void *data, const struct msm_resource_caps_info *avail_res)
{
	...
	struct sde_connector *sde_conn;
	struct msm_resource_caps_info base_avail_res;
	int count;
	int rc;
	u32 edid_size;
	struct edid edid;
	...
	memset(&base_avail_res, 0, sizeof(base_avail_res));
	sde_connector_get_avail_res_info(disp->base->connector, &base_avail_res);

	count = sde_conn->ops.get_modes(disp->base->connector,
				sde_conn->display, &base_avail_res);
	if (!count) {
		SDE_ERROR("no base mode probed\n");
		return 0;
	}

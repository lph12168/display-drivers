	...
	bool disable_border)
{
	...
	if (!disable_border &&
			((!mixercfg[0] && !mixercfg[1] && !mixercfg[0] && !mixercfg[3]) ||
...
static void _sde_shd_hw_ctl_set_fetch_pipe_active(struct sde_hw_ctl *ctx,
		unsigned long *fetch_active)
{
	...
	hw_ctl->active_pipes = val;
	if (val)
		hw_ctl->active_pipes_mask = hw_ctl->active_pipes;
}
	...
	struct sde_hw_mixer *lm_ctx[MAX_MIXERS_PER_CRTC], int lm_num)
{
	...
	/*
	 * CTL_FETCH_PIPE_ACTIVE is shared with each shared display based
	 * on the same display, clear own active pipe settings then set
	 */
	active_pipes = SDE_REG_READ(c, CTL_FETCH_PIPE_ACTIVE);
	active_pipes &= ~hw_ctl->active_pipes_mask;
	active_pipes |= hw_ctl->active_pipes;
	SDE_REG_WRITE(c , CTL_FETCH_PIPE_ACTIVE, active_pipes);
...
void sde_shd_hw_ctl_init_op(struct sde_hw_ctl *ctx)
{
	...
	ctx->ops.set_active_pipes =
		_sde_shd_hw_ctl_set_fetch_pipe_active;
}

...
static DEFINE_MUTEX(g_shp_mutex);
static struct shp_device *g_shp_device[MAX_HW_INSTANCES];

static struct shp_device *get_shp_device(struct drm_device *ddev)
{
	...
	shp_dev = g_shp_device[priv->instance_id];
	if (!shp_dev || ddev != shp_dev->dev) {
		SDE_ERROR("failed to get shp_device %d\n",
				priv->instance_id);
		return NULL;
	}
...
static int shp_atomic_check(struct drm_atomic_state *state)
{
	...
	shp_dev = get_shp_device(state->dev);
	if (!shp_dev) {
		SDE_ERROR("%s: invalid shp device\n", __func__);
		return -EINVAL;
	}
				   ...
				   uint64_t val)
{
	...
	shp_dev = get_shp_device(plane->dev);
	if (!shp_dev) {
		SDE_ERROR("%s: invalid shp device\n", __func__);
		return -EINVAL;
	}
		...
		void *data)
{
	...
	mutex_lock(&g_shp_mutex);
	if (g_shp_device[priv->instance_id]) {
		pr_err("shp device %d has been registered\n",
				priv->instance_id);
		rc = -EINVAL;
		mutex_unlock(&g_shp_mutex);
		goto end;
	}

	g_shp_device[priv->instance_id] = shp_dev;
	mutex_unlock(&g_shp_mutex);
		...
		void *data)
{
	...
	mutex_lock(&g_shp_mutex);
	g_shp_device[priv->instance_id] = NULL;
	mutex_unlock(&g_shp_mutex);
...
static int sde_shp_probe(struct platform_device *pdev)
{
	...
	shp_dev = devm_kzalloc(&pdev->dev, sizeof(*shp_dev), GFP_KERNEL);
	if (!shp_dev) {
		pr_err("failed to malloc shp device\n");
		return PTR_ERR(shp_dev);
	}

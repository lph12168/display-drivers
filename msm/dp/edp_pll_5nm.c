...
#define DP_PHY_VCO_DIV_5NM_7NM		0x0074
...
/* Tx tran offsets */
#define DP_TRAN_DRVR_EMP_EN			0x0078

/* Tx VMODE offsets */
#define DP_VMODE_CTRL1				0x007C
...
#define QSERDES_COM_CMN_MODE			0x01A0
...
static const struct dp_pll_params pll_params[HSCLK_RATE_MAX] = {
	{0x05, 0x3f, 0x00, 0x04, 0x01, 0x69, 0x00, 0x80, 0x07, 0x6f, 0x08, 0x45, 0x06, 0x36, 0x01,
		0x00, 0x00, 0x0f, 0x0a, 0xf, 0x11, 0x11},
...
static int set_vco_div(struct dp_pll *pll, unsigned long rate)
{
	...
	val = dp_pll_read(dp_phy, DP_PHY_VCO_DIV_5NM_7NM);
	val &= ~0x03;
	...
	dp_pll_write(dp_phy, DP_PHY_VCO_DIV_5NM_7NM, val);
...
static void edp_pll_config_tx_5nm(struct dp_pll *pll)
{
...
static void edp_pll_config_tx_7nm(struct dp_pll *pll)
{
	/* TX-0 register configuration */
	dp_pll_write(dp_phy, DP_PHY_TX0_TX1_LANE_CTL, 0x05);
	dp_pll_write(dp_ln_tx0, TXn_TRANSCEIVER_BIAS_EN, 0x03);
		...
		unsigned long rate)
{
	...
	if (pll->ssc_en) {
		...
		dp_pll_write(dp_pll, QSERDES_COM_SSC_EN_CENTER, 0x00);
	}
	...
	dp_pll_write(dp_pll, QSERDES_COM_CMN_MODE, 0x04);
	dp_pll_write(dp_pll, QSERDES_COM_CORE_CLK_EN, 0x0f);
	//dp_pll_write(dp_pll, QSERDES_COM_VCO_TUNE1_MODE0, params->vco_tune1_mode0);
	//dp_pll_write(dp_pll, QSERDES_COM_VCO_TUNE2_MODE0, params->vco_tune2_mode0);
	/* Make sure the PHY register writes are done */
	wmb();

	edp_pll_config_tx_5nm(pll);
...
unsigned long edp_vco_recalc_rate_5nm(struct dp_pll *pll)
{

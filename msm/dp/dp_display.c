...
static int dp_display_sst_validate_mode(
		struct dp_display *dp_display,
...
static int dp_display_mst_validate_mode(
		struct dp_display *dp_display,
		void *panel, struct drm_display_mode *mode,
		const struct msm_resource_caps_info *avail_res)
{
		...
		const struct msm_resource_caps_info *avail_res)
{
	...
	if (dp_display_sst_validate_mode(dp_display, panel, mode, avail_res)) {
		DP_DEBUG("DP SST bad mode %dx%d@%d\n",
			mode->hdisplay, mode->vdisplay, mode->clock);
		goto end;
	}

	if (dp_display_mst_validate_mode(dp_display, panel, mode, avail_res)) {
		DP_MST_DEBUG("DP MST bad mode %dx%d@%d\n",
			mode->hdisplay, mode->vdisplay, mode->clock);
		goto end;
	}
